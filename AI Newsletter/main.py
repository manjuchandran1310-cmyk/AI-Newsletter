...
def main() -> None:
    _check_env("GROQ_API_KEY", "RESEND_API_KEY", "RECIPIENT_EMAIL")
    ...
    # ── 3. Enrich (Gemini API) ────────────────────────────
    print(f"\n[3/5] Enriching {len(articles)} articles via Gemini API...")
    ...
    # Safety net: if Gemini failed for all batches, every article got score=5
    # (set in summarise_all fallback). Still send — never deliver a blank email.
    if not articles:
        print("  → No scored articles — using all fetched articles with default score")
        from scraper.fetch import fetch_all as _fetch_all   # already fetched; reuse list
        # Re-dedupe from cache is not needed; fall through with empty list gracefully
        # (This path only hit if dedupe returned 0 — which was already caught above)
    ...
    from mailer.build import render_email, send_email
